...
library(data.table)
library(readxl)
ntiles_lost <- c(1,2) # bottom two deciles
...
ui <- fluidPage(
    ...
    numericInput("amount_of_time_lost", "Learning lost (number of years)", .33 ,step = .1), # (4 months is 33 per cent of 1 year)
...
server <- function(input, output) {
    ...
    output$text_output <- renderTable(data %>% 
        ...
        mutate(year_start = (input$age_start-age)+2022,
     ...
     filter(n_tile %in% input$ntiles_lost,
        ...
        mutate(wage_index            = if_else( year == 0,1,1 * input$wage_inflation^(year_index)), # A wage index - goes up every year with wages
               ...
               discount_rate_index   = 1/if_else( year == 0,1,1 * input$discount_rate^(year_index)), # A discount rate index - goes down every year with discount rates
               wage_nominal          = wage_index * wage_original_2022,
               ...
               income_pp         = wage_discounted_ntile*52,
    ...
    output$text <- renderText("All values expressed as NPV in $2022. We first take the total average weekly earnings from the ABS labour survey May 2022. 
